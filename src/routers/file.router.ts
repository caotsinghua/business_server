...
import { upload, authenticated, aclAuthenticated } from '../middlewares';
...
fileRouter.post(
    '/saveUploadedDepartmentCustomers',
    authenticated,
    aclAuthenticated,
    async (req: Req, res: Res, next: any) => {
        try {
            const { key } = req.body;
            const job_number = req.session.passport.user;
            const result = await batchCustomersService.saveDepartmentCustomers(key, job_number);
            res.apiSuccess(result);
        } catch (e) {
            next(e);
        }
